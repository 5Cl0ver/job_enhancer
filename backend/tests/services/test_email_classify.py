...
class TestMatch:
    ...
    def test_match_by_sender_domain(self):
        m = match_email_to_job(self.JOBS, company_hint="Acme", subject="Application update")
        assert m and m["id"] == "1"

    def test_match_by_subject_company(self):
        m = match_email_to_job(self.JOBS, subject="Your application to Globex — thank you")
        assert m and m["id"] == "2"
    ...
    def test_no_confident_match_returns_none(self):
        assert match_email_to_job(self.JOBS, company_hint="Umbrella", subject="Hello") is None
