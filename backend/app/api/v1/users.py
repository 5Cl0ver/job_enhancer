...
async def get_profile(user: CurrentUser = Depends()) -> UserProfile:
...
async def export_data(
    ...
    stages = (
        await db.execute(select(PipelineStage).where(PipelineStage.user_id == user.id))
    ).scalars().all()
    ...
    return StreamingResponse(
        ...
        headers={"Content-Disposition": 'attachment; filename="job_enhancer_export.json"'},
    )
...
async def delete_account(
    ...
    """Soft-delete the current user account. All owned records cascade-delete via FK."""
